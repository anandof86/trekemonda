...
const CreateAgentComponent = (props) => {
    ...
    const onSubmit = (data) => {
        if (props.formStatus === 'Add') {
            ...
            axios({
                url: url.server+'api/agent/beta/' + props.currentRow.id,
                method: 'put',
                responseType: 'json',
                data: data,
                headers: {
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
        <div className="row">
            <div className="col-md-6" style={spacing2}>
                <input disabled={disabled} className="form-control" {...register("name", { required: true})} placeholder="Suppiler Name" />
            </div>
            <div className="col-md-6" style={spacing2}>
            ...
            <div className="col-md-6" style={spacing2}>
                <input disabled={disabled} className="form-control" {...register("online_balance", { required: true})} placeholder="Balance" />
            </div>
            <div className="col-md-6" style={spacing2}>
                <select disabled={disabled} className="form-control" {...register("state", { required: true })}>
                        {stateCode.map((value) => (
                            <option key={value.state_code} value={value.state}>{value.state}</option>
                        ))}
                    </select>
            </div>
            <div className="col-md-6" style={spacing2}>
                <input disabled className="form-control" {...register("state_code", { required: true })} placeholder="State Code" />
            </div>
            <div className="col-md-6" style={spacing2}>
                <input className="form-control" {...register("gst_no", { required: true})} placeholder="GST Number" />
            </div>
